...
| Design §1 레이아웃 대비 파일 존재 | ✅ 전 항목 (skills 8, platforms 4+5, engines 3, common 3, setup 2, scripts 2, install.ps1, README) |
...
| G2 | 스킬 미설치 상태 — install.ps1 실행 전 (Claude Code에서 트리거 안 됨) | 중 | install.ps1 실행 |
...
- **잔여**: 재드라이런으로 D4 해제(OPCODE 보정→대사 TSV 생성) 실증 권장, install.ps1 미실행(G2)
