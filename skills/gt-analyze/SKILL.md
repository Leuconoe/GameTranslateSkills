...
5. **번역 대상 분류**: 추출물에서 다음을 식별하여 `ANALYSIS.md`에 목록화:
   ...
   - 매뉴얼/기타: 별도 처리 필요 항목
   - 이미지 범위 판정: 전수 inventory와 시각 확인을 바탕으로 프로젝트의
     `image_scope`를 `required` 또는 `N/A`로 기록한다. 문자 포함 이미지가 번역 범위에
     하나라도 있으면 `required`로 고정하고, 없거나 게임별 분석상 이미지 번역이 적용되지
     않으면 0건 목록과 근거를 함께 남겨 `N/A`로 확정한다. 단순히 이미지를 열어보지 않았다는
     이유로 `N/A`를 지정하지 않는다.
...
- [ ] 텍스트/이미지/폰트 대상 목록이 빠짐없이 작성됨 (전수 조사 — 샘플링 금지).
 ...
 1단계 소관이다. `image_scope` 판정에 필요한 시각 선별을 마친 뒤, 상세 이미지 계획과
  스타일 분석은 4단계(gt-image-translate)의 입력으로 넘긴다.
- [ ] `image_scope`가 `required` 또는 `N/A`로 확정되고 `N/A`인 경우 0건과 판정 근거가
  기록됨
- [ ] 추출이 명령 재실행으로 재현 가능
